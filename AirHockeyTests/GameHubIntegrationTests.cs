...
namespace AirHockeyTests
{
    ...
    public class GameHubIntegrationTests : IAsyncDisposable
    {
        ...
        public class GameHubDriver
        {
            ...
            {
                return _connection.InvokeAsync("UpdateInput", roomCode, playerId, up, down, left, right, attack, false);
            }
        }
        ...
        [Test]
        public async Task LoadTest_Create500RoomsWith2PlayersEach()
        {
            const int ROOM_COUNT = 500;
            const int TEST_DURATION_MINUTES = 5;

            var tasks = new List<Task>();
            var cancellationTokenSource = new CancellationTokenSource();
            cancellationTokenSource.CancelAfter(TimeSpan.FromMinutes(TEST_DURATION_MINUTES));
            ...
            for (int i = 0; i < ROOM_COUNT; i++)
            {
                ...
                tasks.Add(Task.Run(async () =>
                {
                    try
                    {
                        string roomCode = $"ROOM{roomIndex:D3}";
                        string player1Nickname = $"Player1_{roomIndex}";
                        string player2Nickname = $"Player2_{roomIndex}";

                        var player1Driver = new GameHubDriver(await CreateAndConnectHubConnection());
                        var player2Driver = new GameHubDriver(await CreateAndConnectHubConnection());

                        await player1Driver.CreateRoom(roomCode, player1Nickname);
                        await player2Driver.JoinRoom(roomCode, player2Nickname);

                        while (!cancellationTokenSource.Token.IsCancellationRequested)
                        {
                            await player1Driver.UpdateInput(roomCode, "Player1", true, false, false, false, false);
                            await player2Driver.UpdateInput(roomCode, "Player2", false, true, true, false, false);
                    ...
                    {
                        Assert.Fail($"Exception occurred in room {roomIndex}: {ex.Message}");
                    }
                }));
            }
            ...
            Assert.That(GameSessionManager.Instance.ActiveRooms.Count, Is.LessThanOrEqualTo(0), "Not all rooms were properly cleaned up after the test.");
        }

        private async Task<HubConnection> CreateAndConnectHubConnection()
        {
